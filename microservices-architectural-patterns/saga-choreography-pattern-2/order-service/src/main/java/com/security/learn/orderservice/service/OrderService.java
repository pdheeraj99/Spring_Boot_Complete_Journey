...
import com.security.learn.common.dto.*;
...
public class OrderService {
    ...
    // This method now accepts the common interface
    public void confirmOrder(SagaEvent event) {
        orderRepository.findById(event.getOrderId()).ifPresent(order -> {
            if (order.getStatus() == OrderStatus.PENDING) {
                order.setStatus(OrderStatus.CONFIRMED);
                orderRepository.save(order);
                System.out.println("SAGA COMPLETED: Order " + order.getOrderId() + " is CONFIRMED.");
            }
        });
    }


    // This method also accepts the common interface for ANY failure
    public void cancelOrder(SagaEvent event) {
        orderRepository.findById(event.getOrderId()).ifPresent(order -> {
            if (order.getStatus() == OrderStatus.PENDING) {
                order.setStatus(OrderStatus.CANCELLED);
                orderRepository.save(order);
                System.out.println("SAGA ROLLED BACK: Order " + order.getOrderId() + " is CANCELLED.");
            }
        });
    }
}